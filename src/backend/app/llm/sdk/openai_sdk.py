...
def _load_client_class(module_name: str, class_name: str) -> Optional[type]:
    try:
...
def _extract_first_choice(response: Any) -> Optional[Any]:
...
def _extract_message_thinking(message: Any) -> Optional[str]:
    if message is None:
        return None
    if isinstance(message, dict):
...
def _extract_response_payload(response: Any) -> SDKResponse:
    ...
    return SDKResponse(content=content or "", thinking=thinking, usage=usage, metadata=metadata)
