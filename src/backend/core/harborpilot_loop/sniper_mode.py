...
def normalize_cost_class(value: Optional[str]) -> str:
    if not value:
        return "LOCAL"
...
def resolve_cost_class(value: Optional[str] = None) -> str:
    if value:
        return normalize_cost_class(value)
    env_value = (
        os.environ.get("HARBORPILOT_COST_MODEL")
        or os.environ.get("HARBORPILOT_COST_CLASS")
        or ""
    )
