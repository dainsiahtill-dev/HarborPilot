...
def test_new_detection_logic():
    ...
    assert new_check != hasattr(BaseProvider(), 'invoke_stream') or 'invoke_stream' in BaseProvider.__dict__, \
...
def test_simulated_vs_true_streaming_detection():
    ...
    assert 'invoke_stream' in simulated.__class__.__dict__ is False
    assert 'invoke_stream' in true_streaming.__class__.__dict__ is True
