 ...
 */
export function parseHarborPilotChunk(
  chunk: string,
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-950/60">
      <div className="flex items-center justify-between gap-2 border-b border-slate-800 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          {isOpenJson ? <span className="text-xs text-slate-400">parsing...</span> : null}
          {hasError ? <span className="text-xs text-red-300">invalid JSON: {token.error}</span> : null}
        </div>
        <div className="flex items-center gap-2">
  ...
  if (tag === 'exit') {
    ...
    return (
      <div className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2">
        ...
        {code !== '0' ? (
          <span className="text-xs text-red-300">process failed</span>
        ) : (
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        {run.exitCode !== undefined ? <Badge tone={exitTone}>exit {run.exitCode}</Badge> : <Badge>running</Badge>}
      </div>
...
export type HarborPilotTerminalRendererProps = {
 ...
 */
export function HarborPilotTerminalRenderer({ text, className }: HarborPilotTerminalRendererProps) {
  ...
  useEffect(() => {
    ...
    const { tokens: newTokens, state } = parseHarborPilotChunk(
      chunk,
