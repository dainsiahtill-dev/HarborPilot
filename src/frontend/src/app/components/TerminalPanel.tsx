...
import { TerminalSquare, X, Maximize2, Minimize2, Copy, Eraser } from 'lucide-react';
...
export function TerminalPanel({ isVisible, onClose, workspacePath, isMaximized, onToggleMaximize }: TerminalPanelProps) {
    ...
    const initTerminal = async () => {
      try {
        ...
        if (session) {
          ...
          const unsubscribe = window.harborpilot?.pty?.onData(({ id, data }) => {
            if (id === session.id) {
              term.write(data);
            }
          });
