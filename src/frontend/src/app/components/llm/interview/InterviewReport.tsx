    ...
    <div className="rounded-lg border border-white/10 bg-black/30 p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-text-main font-semibold">{roleLabel} Interview Report</span>
        </div>
        {score !== null ? (
          <div className="text-xs text-text-dim">
            Final Score: <span className="text-text-main font-semibold">{score}</span>
          </div>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
        <div className="rounded-md border border-white/10 bg-black/20 p-3">
          <div className="text-[10px] uppercase tracking-wide text-text-dim mb-1">Thinking Capability</div>
          <div className="text-text-main font-semibold">
            {thinkingSupport ? 'Supported' : 'Not detected'}
          </div>
          <div className="text-text-dim">
            Confidence: {thinkingConfidence !== undefined ? Math.round(thinkingConfidence * 100) : 'n/a'}%
          </div>
        </div>
        <div className="rounded-md border border-white/10 bg-black/20 p-3">
          <div className="text-[10px] uppercase tracking-wide text-text-dim mb-1">Recommendation</div>
          <div className="text-text-main font-semibold">
            {report.details?.recommendation || 'No recommendation yet.'}
          </div>
