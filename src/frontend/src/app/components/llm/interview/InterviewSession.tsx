  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs text-text-dim uppercase tracking-wide">Interview Session</div>
          <h3 className="text-lg font-semibold text-text-main">
            面试进行中: {roleLabel} 岗位
          </h3>
          <div className="text-[10px] text-text-dim">Role ID: {roleId}</div>
        </div>
        {onBack ? (
          <button
            onClick={onBack}
      ...
      {running ? (
        <div className="rounded-lg border border-white/10 bg-black/20 p-6 text-center text-sm text-text-dim">
          ...
          Interview in progress. Gathering responses...
        </div>
      ) : null}
      ...
      {!running && report ? (
        <>
          <ThinkingDisplay
            title="Thinking Capability Snapshot"
            ...
            {cases.map((item, idx) => (
              <div key={item.id || idx} className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
                ...
                <div className="rounded-lg border border-white/10 bg-black/20 p-3 text-xs">
                  ...
                  <div className="text-text-main">{item.notes || 'Evaluation pending.'}</div>
                  {Array.isArray(item.criteria_hits) && item.criteria_hits.length > 0 ? (
                    <div className="text-text-dim mt-2">
                      Matched: {item.criteria_hits.join(', ')}
                    </div>
                  ) : null}
