                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {entries.map((provider) => (
                    <button
                      ...
                      disabled={isSaving}
                      className="text-left rounded-xl border border-white/10 bg-black/20 hover:border-white/30 transition-all disabled:opacity-60 p-3"
                    >
      ...
      {hasConfiguredProviders ? (
        <div className="space-y-3">
          {Object.entries(configuredProviders).map(([providerId, provider]) => {
            ...
            const connectivityStatus = useConnectivityStatus(providerId);
