...
本文档定义了 HarborPilot 系统中所有大语言模型（LLM）提供商必须遵循的接入规范，确保新模型接入时能够保持架构一致性，特别是思考过程解析、明文输入处理、实时流式对话等核心特性的统一支持。
...
思考过程（Thinking Process）是 LLM 拟人化能力的核心组成部分。在 HarborPilot 系统中，思考内容被用于"自言自语"（Inner Voice）功能，让用户能够观察 AI 的推理过程，增强交互的真实感和透明度。因此，**所有提供商都必须实现思考内容提取能力**，无论原 API 是否原生支持思考标记。
...
HarborPilot 采用**本地优先（Local-First）**架构，所有输入框默认采用明文显示，不考虑安全性问题。这一设计决策基于以下考量：本地环境的安全性由用户自身保证、API Key 等敏感信息需要被用户清晰识别、明文显示便于调试和配置确认。

与大多数云端应用不同，HarborPilot 不需要防止" hombroder 看到敏感信息"，因为运行环境是用户自己的设备。过度隐藏这些信息只会增加配置难度和用户困惑。
...
实时流式对话是 HarborPilot 架构的核心需求之一。在传统的请求-响应模式中，用户需要等待整个请求完成才能看到任何输出，这对于长时间运行的推理任务来说是不可接受的体验。流式响应让用户能够**实时观察 AI 的思考过程和输出进度**，大大提升交互的透明度和满意度。
...
HarborPilot 根据不同运行环境定义了成本模型，Provider 实现应该感知并适配这些模型。

| 模式 | 特征 | 适配策略 |
|-----|------|---------|
| `LOCAL` | 本地模型（Ollama），上下文窗口小 | 极简主义，减少上下文、批量操作 |
| `FIXED` | 配额限制（Copilot CLI） | 批量化请求，减少往返次数 |
| `METERED` | 按 Token 计费（GPT-4, Claude） | 严格压缩，禁用冗余输出 |

### 7.2 上下文优化

对于本地模型，应该主动压缩上下文长度，避免超出模型限制。可以实现一个简单的上下文裁剪策略。
...
本文档将随着 HarborPilot 架构演进持续更新。更新内容包括：新特性支持规范、现有规范的优化调整、新的 Provider 接入案例。
...
*本文档由 HarborPilot Architecture Team 维护*
