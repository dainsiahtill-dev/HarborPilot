# HarborPilot Context Engine v2 升级计划

目标：把“正确骨架”的上下文管理升级为**可靠、可运营、可回放**的企业级系统，吸收主流 Agent 的关键能力，同时保持 HarborPilot 宪法约束（合同不可变、事件流追加、run_id 全局唯一、UI 只读、可回放、失败可定位、Atomic IO、Memory refs）。
...
- 路径：`.harborpilot/runtime/runs/<run_id>/evidence/context_snapshot_<hash>.json`
...
长输出写入 `.harborpilot/runtime/runs/<run_id>/evidence/*.txt`：

```python
def artifact_long_output(content: str, run_id: str, kind: str) -> str:
    artifact_dir = f".harborpilot/runtime/runs/{run_id}/evidence"
