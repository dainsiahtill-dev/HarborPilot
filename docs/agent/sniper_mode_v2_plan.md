# HarborPilot 上下文工程优化计划（Sniper Mode v2.0）

基于 HarborPilot 三层成本模型，实施 **Structure-Aware 上下文工程优化**，重点解决上下文窗口管理与 **METERED** 模式成本控制问题。
...
```json
{
  "system_prompt": "你使用 HarborPilot Sniper Mode v2.0 进行上下文工程优化。\\n\\n成本感知策略：\\n- LOCAL 模式：优先上下文窗口管理，避免溢出\\n- FIXED 模式：优化配额利用率\\n- METERED 模式：严格控制 Token 消耗\\n\\n工作流程：\\n1. 检测成本模型\\n2. 选择对应策略\\n3. 执行精准定位和修改"
}
```
...
HarborPilot Sniper Mode v2.0 的核心价值是**上下文工程优化**。通过 Structure-Aware 技术解决不同成本模型的核心痛点：

- **LOCAL 模式**：上下文窗口管理与推理效率  
- **FIXED 模式**：配额优化与利用率提升  
- **METERED 模式**：成本控制与费用节省

该方案符合 HarborPilot“本地优先、固定成本优先”的设计理念，是实现“单人低成本长跑”愿景的关键技术升级。
