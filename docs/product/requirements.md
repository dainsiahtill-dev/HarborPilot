...
HarborPilot 是一个**单人本地优先**的自动化编程指挥台，要求对接的 LLM 模型具备**思考过程**和**流式输出**能力。
...
3. **固定成本优先的运营模式**：通过 LOCAL/FIXED 通道优先，降低边际成本
...
**验收标准**：
- LOCAL 通道：本地模型（Ollama 等），边际成本 ≈ 电费
...
| Role | HarborPilot 中的角色（PM/Director/QA/Docs） |
| Run ID | 全局唯一的运行标识符 |
