...
def maybe_generate_agents_draft(
    ...
    def _build_fallback(docs_text: str, root_text: str, feedback_text: str, error_hint: str = "") -> str:
        ...
        if docs_text:
            sections.append("## docs/README.md (excerpt)\n" + truncate_text_block(docs_text))
        if root_text:
        ...
        content = (
            ...
            "- Keep state artifacts in state/ollama/ (ramdisk may be enabled).\n"
    ...
    try:
        ...
        if backend == "codex":
            codex_output = resolve_artifact_path(
                workspace_full,
                cache_root_full,
                "state/ollama/AGENTS.codex.last_message.md",
            )
            output = invoke_codex(
                prompt,
                codex_output,
