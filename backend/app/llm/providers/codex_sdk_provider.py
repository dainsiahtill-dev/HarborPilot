...
from .base_provider import BaseProvider, ProviderInfo, ValidationResult, ThinkingInfo
from ..sdk import CodexSDK, SDKConfig, SDKMessage, SDKUnavailableError
...
def _normalize_retries(value: Any, default: int = 3) -> int:
    try:
        value = int(value)
        return value if value >= 0 else default
    except Exception:
...
def _usage_from_sdk(prompt: str, output: str, usage: Optional[Dict[str, Any]]) -> Usage:
...
def _build_sdk_config(config: Dict[str, Any]) -> SDKConfig:
    ...
    return SDKConfig(
        ...
        max_retries=_normalize_retries(config.get("max_retries"), 3),
...
class CodexSDKProvider(BaseProvider):
    ...
    def __init__(self) -> None:
        self._sdk_client: Optional[CodexSDK] = None
        self._sdk_key: Optional[Tuple[Any, ...]] = None
    ...
    def validate_config(cls, config: Dict[str, Any]) -> ValidationResult:
        ...
        if max_retries != config.get("max_retries"):
            warnings.append("Invalid max_retries, using default 3")
    ...
    def _sdk_identity(self, config: Dict[str, Any]) -> Tuple[Any, ...]:
        headers = _normalize_headers(config.get("headers")) or {}
    ...
    def invoke(self, prompt: str, model: str, config: Dict[str, Any]) -> InvokeResult:
        ...
        try:
            ...
            if config.get("temperature") is not None:
                sdk_kwargs["temperature"] = float(config.get("temperature"))
            if config.get("max_tokens") is not None:
                sdk_kwargs["max_tokens"] = int(config.get("max_tokens"))
