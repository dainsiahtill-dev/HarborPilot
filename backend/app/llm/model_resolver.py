...
def validate_model_name(model: str, provider_type: Optional[str] = None) -> ModelValidationResult:
    """
    ...
    if not stripped:
        return ModelValidationResult(
            is_valid=False,
            error="Model name cannot be empty after stripping whitespace"
        )
    ...
    for char in invalid_chars:
        if char in stripped:
            return ModelValidationResult(
                is_valid=False,
                error=f"Model name contains invalid character: {char}"
            )
...
def get_default_model_for_provider(provider_type: str) -> Optional[str]:
