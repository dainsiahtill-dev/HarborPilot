# HarborPilot Stage B Test Plan (Functional)

## Purpose
Stage B validates real functional behavior across HarborPilot workflows. These tests exercise the loops end‑to‑end with controlled inputs and (optionally) real tools/models.

## Execution Guard
Default Stage B tests run by default (mocked models, controlled inputs).
Real Ollama integration tests are opt-in:
- Set `HARBORPILOT_OLLAMA_MODEL` to a locally installed model (e.g. `qwen2.5-coder:latest`).
- Optional: `HARBORPILOT_OLLAMA_TIMEOUT` to cap runtime (seconds).
...
- Behavior:
  ...
  - Skips automatically when `HARBORPILOT_OLLAMA_MODEL` is not set or the model is not installed
