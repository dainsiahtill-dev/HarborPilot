...
export function InterventionCenter({ isOpen, onClose }: InterventionCenterProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'ignored':
        return <Badge variant="outline" className="bg-gray-500/10 text-gray-500 border-gray-500/20">Ignored</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col bg-[#1e1e1e] border-gray-800 text-gray-200 p-0 gap-0">
          ...
          <div className="flex items-center justify-between">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-[200px]">
              <TabsList className="grid w-full grid-cols-2 bg-gray-800">
                <TabsTrigger value="pending">Pending ({interventions.filter(i => i.status === 'pending').length})</TabsTrigger>
                <TabsTrigger value="history">History</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
          <DialogDescription className="text-gray-400 mt-2">
            Centralized control for manual approvals, policy blocks, and critical system alerts.
          </DialogDescription>
        </DialogHeader>
            ...
            <ScrollArea className="flex-1">
              <div className="p-4 space-y-2">
                {loading ? (
                  ...
                  <div className="text-center text-gray-500 py-8">No interventions found.</div>
                ) : (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          {getIcon(item.type)}
                          <span className="text-xs font-medium text-gray-300 truncate max-w-[120px]">{item.type.replace('_', ' ').toUpperCase()}</span>
                        </div>
          ...
          <div className="flex-1 flex flex-col bg-[#141414]">
            {selectedIntervention ? (
              <>
                <ScrollArea className="flex-1 p-6">
                  <div className="space-y-6">
                    ...
                    {selectedIntervention.context && (
                      <div className="bg-gray-800/30 rounded-lg p-4 border border-gray-800">
                        <h3 className="text-sm font-medium text-gray-300 mb-2">Context Data</h3>
                ...
                <div className="p-6 border-t border-gray-800 bg-[#1e1e1e]">
                  <div className="flex items-center justify-end gap-3">
                    {selectedIntervention.status === 'pending' ? (
                      selectedIntervention.actions?.map((action) => (
                        ...
                        <>
                          <Button variant="ghost" onClick={() => handleAction(selectedIntervention.id, 'ignore')}>Ignore</Button>
                          <Button variant="default" onClick={() => handleAction(selectedIntervention.id, 'approve')}>Approve</Button>
                        </>
                      )
                    ) : (
                      <div className="text-sm text-gray-500 italic">
                        This item has been {selectedIntervention.status}.
                      </div>
                    )}
                  </div>
                </div>
