  ...
  return (
    <header className="h-14 bg-[#252526] border-b border-gray-800 flex items-center justify-between px-4">
      ...
      <div className="flex-1 max-w-md mx-8 relative">
        <div
          className={`flex items-center gap-2 bg-[#1e1e1e] rounded px-3 py-1.5 border ${workspaceError ? 'border-red-500/60' : 'border-gray-700'
            }`}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1e1e1e] rounded border border-gray-700">
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors relative ${pmRunning
          ...
          {onRunPmOnce ? (
            <button
              ...
              className={`p-1.5 rounded transition-colors bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 relative ${runOnceBlocked || isStartingPM ? 'opacity-50 cursor-not-allowed hover:bg-transparent' : ''
                }`}
              title="运行一次"
            >
          ...
          {onResumePm && !pmRunning ? (
            <button
              onClick={onResumePm}
              disabled={pmDisabled || isStartingPM || isStoppingPM}
              className={`p-1.5 rounded transition-colors bg-amber-500/20 text-amber-400 hover:bg-amber-500/30 relative ${pmDisabled || isStartingPM || isStoppingPM ? 'opacity-50 cursor-not-allowed hover:bg-transparent' : ''
                }`}
              title="恢复上次运行"
            >
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1e1e1e] rounded border border-gray-700">
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors relative ${directorRunning
                ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30'
                : 'bg-green-500/20 text-green-400 hover:bg-green-500/30'
        ...
        {showAgents ? (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1e1e1e] rounded border border-amber-500/30">
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors relative ${agentsReady
        ...
        {onStopOllama ? (
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[#1e1e1e] rounded border border-gray-700">
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors bg-red-500/20 text-red-400 hover:bg-red-500/30 relative ${isStoppingOllama ? 'opacity-50 cursor-not-allowed hover:bg-transparent' : ''
                }`}
              title="停止 Ollama 模型"
            >
