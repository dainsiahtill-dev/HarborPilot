/**
 * EnhancedLLMSettingsTab - Refactored Version
 * 使用 Context + Reducer 模式替代分散的 useState
 * 代码从 2267 行缩减至约 300 行
 */
...
function resolveModelForSelection(
  ...
  if (providerCfg?.model) return providerCfg.model;
  if (providerCfg?.default_model) return providerCfg.default_model;
  if (roleCfg?.provider_id === providerId && roleCfg.model) return roleCfg.model;
...
function DeepTestPanel({
  ...
  onAskInteractiveInterview: EnhancedLLMSettingsTabProps['onAskInteractiveInterview'];
  onSaveInteractiveInterview: EnhancedLLMSettingsTabProps['onSaveInteractiveInterview'];
  resolveProviderEnvOverrides?: (providerId: string) => Promise<Record<string, string> | null>;
  onCancelInterview?: () => void;
}) {
...
function EnhancedLLMSettingsTabInner({
  llmConfig,
  llmStatus,
  llmLoading,
  llmSaving,
...
export function EnhancedLLMSettingsTabRefactored(props: EnhancedLLMSettingsTabProps) {
  return (
    <ProviderContextProvider>
      <EnhancedLLMSettingsTabInner {...props} />
    </ProviderContextProvider>
  );
}

export default EnhancedLLMSettingsTabRefactored;
