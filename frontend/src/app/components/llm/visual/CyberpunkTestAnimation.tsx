...
export function CyberpunkCard({ children, status, className = '' }: CyberpunkCardProps) {
  const statusColors = {
    running: {
  ...
  if (status === 'running') {
    return (
      <div className={`relative overflow-hidden rounded-xl ${colors.border} ${colors.bg} ${colors.glow} ${className}`}>
  ...
  return (
    <div className={`rounded-xl border ${colors.border} ${colors.bg} ${colors.glow} ${className}`}>
      {children}
    </div>
  );
}
