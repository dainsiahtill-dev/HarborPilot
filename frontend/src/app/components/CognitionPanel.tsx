...
export function CognitionPanel({ events, loading, anthroState }: CognitionPanelProps) {
    ...
    return (
        <div className="h-full flex flex-col glass border-l border-white/5 bg-gradient-to-b from-[#1e1e1e] to-[#0f0f0f] relative overflow-hidden">
          ...
          ${mood.text === 'Confident' ? 'bg-green-500' : mood.text === 'Cautious' ? 'bg-yellow-500' : 'bg-red-500'}`}
            />
            ...
            <div className="px-5 py-4 border-b border-white/5 bg-white/5 backdrop-blur-md sticky top-0 z-20">
                ...
                <div className="flex items-center justify-between mb-4 mt-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-heading font-bold text-gray-100 tracking-wide">Glass Mind</h2>
                            <div className="flex items-center gap-2 text-[10px] text-gray-500 font-mono mt-0.5">
                                ...
                                <span>•</span>
                                <span>Step {latestStats.step}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex p-1 bg-black/20 rounded-lg border border-white/5">
                    ...
                    >
                        <Zap className="size-3" /> Recall Stream
                    </button>
                    <button
                        ...
                        >
                            {recallEvents.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-40 text-gray-600 gap-2">
                                    ...
                                    <span className="text-xs">No cognition events yet</span>
                                </div>
                            ) : (
                        ...
                        >
                            {reflectionEvents.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-40 text-gray-600 gap-2">
                                    ...
                                    <span className="text-xs">No reflections generated yet</span>
                                </div>
                            ) : (
...
function ThoughtChainHUD({ phase }: { phase: string }) {
    const steps = ['Observe', 'Recall', 'Reflect', 'Act'];
            ...
            <div className="space-y-2">
                ...
                <div>
                    ...
                    {context.retrieved_mem_ids && context.retrieved_mem_ids.length > 0 ? (
                        ...
                        <span className="text-[10px] text-gray-700 italic">No memories relevant</span>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/5">
                    ...
                    <span className="text-[10px] text-blue-400/80 font-mono">{context.strategy || 'hybrid'}</span>
                </div>
            </div>
        </motion.div>
    );
        ...
        >
            ...
            <div className="flex items-start gap-3 relative z-10">
                ...
                <div className="flex-1">
                    <h4 className="text-xs font-bold text-gray-200 uppercase tracking-widest mb-1 opacity-80">Insight</h4>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-[10px]">
                        ...
                        <span className="ml-auto flex items-center gap-1 text-gray-500">
                            Confidence: {Math.round((reflection.confidence || 0) * 100)}%
                        </span>
                    </div>
                </div>
            </div>
