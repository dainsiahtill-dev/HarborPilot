...
export function validateApiPayload<T extends keyof InterviewApiEndpoints>(
  ...
  if (!rules) {
    if (ENABLE_VALIDATION_LOGGING) {
      console.warn(`[API Validation] No validation rules for endpoint: ${endpoint}`);
    }
    ...
    if (options?.body && isValidatedEndpoint(endpoint)) {
      try {
        ...
        if (e instanceof SyntaxError) {
          console.warn(`[API Validation] Invalid JSON in request body for ${endpoint}`);
        } else {
...
export function setValidationLogging(enabled: boolean): void {
  (globalThis as any).__API_VALIDATION_LOGGING__ = enabled;
}
...
export function isValidationLoggingEnabled(): boolean {
  return (globalThis as any).__API_VALIDATION_LOGGING__ ?? ENABLE_VALIDATION_LOGGING;
}
